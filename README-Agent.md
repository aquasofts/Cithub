...
core-captcha   可选验证码自动化能力契约与手动版空实现
core-ui        Material 3 主题与通用组件
core-webvpn    接口模型、RSA 加密、CookieJar、Keystore 会话、仓库层
feature-captcha-autofill  ML Kit 本机验证码识别实现（仅自动版依赖）
docs           WebVPN 协议核对记录
```
...
只有服务端明确返回 400/401/402 才清除无效 Cookie；断网、超时或服务端临时故障不会误删本地会话。`autoCaptcha` 版本在 WebVPN 或教务会话明确失效后，会使用最近一次成功登录的加密凭据和自动识别的新验证码重新登录，最多尝试 10 次，每次间隔 1 秒。`manualCaptcha` 版本不识别或回填验证码，也不执行验证码自动续登。

用户可主动勾选“在本机加密保存此账号和密码”。保存后可从登录页选择账号；`autoCaptcha` 版本会自动识别并填写验证码，`manualCaptcha` 版本要求手动输入。最多保留最近使用的 10 个账号，可随时单独删除。

## 安全约束

- 不提交真实账号、密码、Cookie、JWT、token、手机号、姓名、学号或验证码图片。
- 密码只在内存中使用学校公开 RSA 公钥和 `RSA/ECB/PKCS1Padding` 加密后提交。
- Cookie、`autoCaptcha` 版本最近一次成功登录的保活凭据和用户主动保存的账号密码均使用 Android Keystore 的 AES-GCM 密钥加密后写入 DataStore；不再保存认证 token。`manualCaptcha` 版本会清除保活凭据。
- 最近一次成功登录凭据只用于掉线自动保活，主动退出时清除；只有用户明确勾选的账号才进入可选择的账号列表，UI 状态和账号列表中不包含密码明文。
- 网络日志默认关闭；退出登录时清除会话和自动保活凭据。
- `autoCaptcha` 版本的图形验证码仅在本机通过 ML Kit 识别并回填，用户可修改；`manualCaptcha` 版本不包含 ML Kit 识别模块。两种版本都不把验证码图片发送给第三方服务，也不绕过二次认证、改密、绑定账号等安全流程。
...
`assembleDebug` 和 `assemblePerformance` 会分别同时构建两种验证码风味：

- `app-autoCaptcha-<buildType>.apk`：包含本机识别、自动回填和验证码自动续登。
- `app-manualCaptcha-<buildType>.apk`：仅展示验证码并由用户手动输入，不打包 ML Kit。

需要单独构建时可使用 `assembleAutoCaptchaPerformance` 或 `assembleManualCaptchaPerformance`。

## 学生成绩流程

WebVPN 登录成功后，应用会使用同一个加密 Cookie 会话访问学校教学一体化服务平台。学生端登录有独立验证码；自动版会在本机识别并填写，手动版由用户输入。若用户此前主动保存过同名 WebVPN 账号密码，可选择直接使用本机加密凭据，否则需要再次输入教务密码。
